# Makefile for logstore2parquet
#
# Based on Vincent Bernat's blog post and companion GitHub repo:
...
.PHONY: build
build: ; $(info $(M) building executable with golang:latest…) @ ## Build program binary using the latest golang
	$Q docker run -v $(PWD):/l2p golang:latest bash -c "cd /l2p; make; chown $$(id -u):$$(id -u) /l2p/bin/*"
...
TEST_TARGETS := test-default test-bench test-short test-verbose test-race test-units
.PHONY: $(TEST_TARGETS) test-xml check test tests
test-bench:   ARGS=-run=__absolutelynothing__ -bench=. ## Run benchmarks
test-short:   ARGS=-short        ## Run only short tests
test-verbose: ARGS=-v            ## Run tests in verbose mode with coverage reporting
test-race:    ARGS=-race         ## Run tests with race detector
test-units:   ARGS=-v -tags unit ## Run tests with no build tag or the unit tag
$(TEST_TARGETS): NAME=$(MAKECMDGOALS:test-%=%)
$(TEST_TARGETS): test
check test tests: fmt lint ; $(info $(M) running $(NAME:%=% )tests…) @ ## Run tests
	$Q env GO_TEST=1 $(GO) test -timeout $(TIMEOUT)s $(ARGS) $(TESTPKGS)

test-xml: fmt lint | $(GO2XUNIT) ; $(info $(M) running xUnit tests…) @ ## Run tests with xUnit output
	$Q mkdir -p test
	$Q 2>&1 env GO_TEST=1 $(GO) test -timeout $(TIMEOUT)s -v $(TESTPKGS) | tee test/tests.output
